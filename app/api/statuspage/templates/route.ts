...
import { trackTemplate, isDemoMode } from '@/lib/demo-tracker';
...
export async function GET(request: NextRequest) {
  ...
  try {
    const response = await fetch(
      `${API_BASE_URL}/pages/${STATUSPAGE_PAGE_ID}/incident_templates`,
      {
        headers: {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Track created template in demo mode
    if (isDemoMode() && data.id) {
      await trackTemplate(data.id);
    }
